...
<div class="pull-left">
   <?php
    if ($count) {
        ...
        echo sprintf(__('%s does not have any tasks'), $ticket? __('Este expediente') :
                __('System'));
    }
   ?>
</div>
...
<?php csrf_token(); ?>
 ...
 <table class="list" border="0" cellspacing="1" cellpadding="2" width="1120">
    <thead>
        <tr>
            <?php
            if (1) {?>
    ...
    foreach($tasks as $task) {
        ...
        $status = $task->isOpen() ? '<strong>'.__('Abierto').'</strong>': __('Closed');
