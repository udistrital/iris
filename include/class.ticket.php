...
implements RestrictedAccess, Threadable, Searchable {
   ...
   function onOverdue($whine=true, $comments="") {
    ...
    function unlinkChild($parent) {
        ...
        $this->save();
        $this->logEvent('unlinked', array('ticket' => sprintf('Expediente #%s', $parent->getNumber()), 'id' => $parent->getId()));
        $parent->logEvent('unlinked', array('ticket' => sprintf('Expediente #%s', $this->getNumber()), 'id' => $this->getId()));
    }
                    ...
                    if ($parent && $parent->getId() != $ticket->getId()) {
                        if (($changeParent) || ($parent->isParent() && $ticket->getMergeType() == 'visual' && !$ticket->isChild()) || //adding to link/merge
                           (!$parent->isParent() && !$ticket->isChild())) { //creating fresh link/merge
                               $parent->logEvent($eventName, array('ticket' => sprintf('Expediente #%s', $ticket->getNumber()),  'id' => $ticket->getId()));
                               $ticket->logEvent($eventName, array('ticket' => sprintf('Expediente #%s', $parent->getNumber()),  'id' => $parent->getId()));
    ...
    function delete($comments='') {
        ...
        // Log delete
        $log = sprintf(__('Expediente #%1$s eliminado por %2$s'),
            $this->getNumber(),
        ...
        $ost->logDebug(
            sprintf( __('Expediente #%s eliminado'), $this->getNumber()),
            $log
        );
        return true;
    }
    ...
    function update($vars, &$errors) {
        ...
        $vars['note'] = ThreadEntryBody::clean($vars['note']);
        if ($vars['note'])
            $this->logNote(_S('Expediente actualizado'), $vars['note'], $thisstaff);
   ...
   /*============== Static functions. Use Ticket::function(params); =============nolint*/
    ...
    static function create($vars, &$errors, $origin, $autorespond=true,
        ...
        $reject_ticket = function($message) use (&$errors) {
            ...
            $ost->logWarning(_S('Expediente rechazado'), $message, false);
            return 0;
        };
        ...
        switch (strtolower($origin)) {
            ...
            case 'staff':
                ...
                $fields['duedate']  = array('type'=>'date', 'required'=>0, 'error'=>__('Invalid date format - must be MM/DD/YY'));
            case 'api':
                $fields['source']   = array('type'=>'string', 'required'=>1, 'error'=>__('Indique la fuente del expediente'));
                break;
            case 'email':
            ...
            default:
                ...
                $errors['err']=$errors['origin'] = __('El origen indicado para el expediente no es válido');
        }
        ...
        if (!$errors) {
            ...
            try {
                ...
                return $reject_ticket(
                    sprintf(_S('Expediente rechazado (%s) por el filtro "%s"'),
            ...
            if ($cfg->getMaxOpenTickets() > 0
                ...
                $ost->logWarning(sprintf(_S('Expediente rechazado - %s'), $vars['email']),
                        sprintf(_S('Máximo de expedientes abiertos (%1$d) alcanzado para %2$s'),
            ...
            if (!$user) {
                ...
                if (strcasecmp($origin, 'email') === 0
                    ...
                    if (!Organization::forDomain($domain)) {
                        return $reject_ticket(
                            sprintf(_S('Expediente rechazado (%s) (cliente no registrado)'),
                                $vars['email']));
                    }
                }
        ...
        if ($_SESSION[':form-data']['ticketId'] || $_SESSION[':form-data']['taskId']) {
          ...
          $note = array(
                  'title' => __('Expediente creado desde una entrada del hilo'),
                  'body' => sprintf(__(
                        // %1$s is the word Ticket or Task, %2$s will be a link to it
                        'Este expediente fue creado desde %1$s %2$s'),
          ...
          $ticketNote = array(
              'title' => __('Expediente creado desde una entrada del hilo'),
              'body' => sprintf(__('Expediente %1$s<br/> Entrada del hilo: %2$s'),
                $ticketLink, $entryLink)
          );

          $taskNote = array(
              'title' => __('Expediente creado desde una entrada del hilo'),
              'note' => sprintf(__('Expediente %1$s<br/> Entrada del hilo: %2$s'),
                $ticketLink, $entryLink)
          );
        ...
        if ($ticket->isOpen()) {
            ...
            if ($vars['assignId']) {
                ...
                $username = __('Filtro de expedientes');
                if ($vars['staffId'])
    ...
    static function open($vars, &$errors) {
        ...
        if ($vars['deptId']
            ...
            $errors['err'] = sprintf(__('No tiene permiso para crear un expediente en %s'), __('this department'));
            return false;
        }
        ...
        if ($vars['assignId'] && !(
            ...
            $errors['assignId'] = __('Acción denegada. No tiene permiso para asignar o reasignar expedientes.');
        }
        ...
        if (!$vars['assignId'] && $vars['note']) {
            ...
            $ticket->logNote(_S('Nuevo expediente'), $vars['note'], $thisstaff, false);
        }
